...
void OcrEngine::recognizeImpl(const QPixmap &pixmap, const QString &languageTag,
    ...
    if (languageTag.compare(QStringLiteral("auto"), Qt::CaseInsensitive) == 0) {
        ...
        if (withLayout) {
            // Для экранного перевода важнее геометрия строк, чем автоопределение языка:
            // Tesseract auto/OSD + TSV менее надёжны, поэтому берём предпочтительный язык.
            startRecognitionProcess(imagePath, td, preferred, true);
            return;
        }
                                          ...
                                          const QString &preferredLanguage)
{
    ...
    auto continueWithScript = [this, imagePath, tessdataDirectory, installed,
        ...
        qInfo() << "[EShot] OCR automatic selection script=" << script
                << "languages=" << languages;
        startRecognitionProcess(imagePath, tessdataDirectory, languages, false);
    };
